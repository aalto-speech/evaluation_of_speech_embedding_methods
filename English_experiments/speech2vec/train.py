    ...
    for epoch in range(100):
        if early_stopping == False:
            ...
            for iteration, batch in enumerate(pairs_batch_train):
                ...
                encoder_hidden = (encoder_hidden[0].sum(0, keepdim=True), encoder_hidden[1].sum(0, keepdim=True))

                # generate the previous word
                decoder_hidden = encoder_hidden
                decoder_input = torch.zeros((1, decoder_hidden[0].size(1), 13)).to(device)
                out_seqs_1 = torch.zeros(pad_prev_label_seqs.size(0), pad_prev_label_seqs.size(1), pad_prev_label_seqs.size(2)).to(device)
                
                for i in range(0, pad_prev_label_seqs.size(0)):
                    output, decoder_hidden = decoder(encoder_hidden, decoder_input, decoder_hidden)
                    decoder_input = pad_prev_label_seqs[i].unsqueeze(0)
                    out_seqs_1[i] = decoder.out(output)
                
                # generate the next word
                decoder_hidden = encoder_hidden
                decoder_input = torch.zeros((1, decoder_hidden[0].size(1), 13)).to(device)
                out_seqs_2 = torch.zeros(pad_next_label_seqs.size(0), pad_next_label_seqs.size(1), pad_next_label_seqs.size(2)).to(device)

                for i in range(0, pad_next_label_seqs.size(0)):
                    output, decoder_hidden = decoder(encoder_hidden, decoder_input, decoder_hidden)
                    decoder_input = pad_next_label_seqs[i].unsqueeze(0)
                    out_seqs_2[i] = decoder.out(output)
                           
                
                train_loss = criterion(out_seqs_1, pad_prev_label_seqs) + criterion(out_seqs_2, pad_next_label_seqs)
            ...
            with torch.no_grad():
                ...
                for _, batch in enumerate(pairs_batch_dev):
                    ...
                    dev_loss = criterion(out_seqs_1, pad_prev_label_seqs) + criterion(out_seqs_2, pad_next_label_seqs)
