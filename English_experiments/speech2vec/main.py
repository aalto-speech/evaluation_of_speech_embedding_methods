...
'''
print("Loading data..")
features_train = prepare_data.load_features_segmented_combined("path/to/data.npy", form_triplets=True)
features_dev = prepare_data.load_features_segmented_combined("path/to/data.npy", form_triplets=True)
print("Done...")
...
if skip_training == False:
    ...
    #checkpoint = torch.load("weights/new/state_dict_75.pt")
    ...
    checkpoint = torch.load("weights/optimised/state_dict_21.pt", map_location=torch.device("cpu"))
    encoder.load_state_dict(checkpoint["encoder"])
    decoder.load_state_dict(checkpoint["decoder"])
    encoder.eval()
    decoder.eval()
...
# features_dev = prepare_data.load_features_segmented_combined("path/to/data", form_triplets=False)
# with open("../../data/LibriSpeech/transcripts/test.txt", "r") as f:
